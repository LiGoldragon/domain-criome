...
impl OrdinaryFrameIo {
    pub fn read(reader: &mut impl Read) -> Result<signal_domain_criome::Frame> {
    ...
    pub fn write(writer: &mut impl Write, frame: &signal_domain_criome::Frame) -> Result<()> {
...
impl OwnerFrameIo {
    pub fn read(reader: &mut impl Read) -> Result<owner_signal_domain_criome::Frame> {
        let bytes = FrameBytes::read_from(reader)?;
    ...
    pub fn write(writer: &mut impl Write, frame: &owner_signal_domain_criome::Frame) -> Result<()> {
        let bytes = frame.encode_length_prefixed()?;
        writer.write_all(&bytes)?;
        Ok(())
    }
